...
result['min'] = result.close.rolling(window,center=True).min()
result['max'] = result.close.rolling(window,center=True).max()
...
#数趋势-------------------------------------=---------------------------------------
pre_min = result.close[0]
pre_value = result.close[0]
...
for i in range(1,len(result)):
    if (result.close[i] == result['min'][i]) & (result.close[i] != result.close[i-1]):
        trend_rate = (pre_value - result.close[i])/pre_value
        trend_len = length
        trend_mark = 'down'
        pre_value = result.close[i]
        length = 1
        result_trend_counter.append([trend_mark,trend_len,trend_rate,result['date'][i],pre_value])
    elif (result.close[i] == result['max'][i]) & (result.close[i] != result.close[i-1]):
        trend_rate = (result.close[i] - pre_value)/pre_value
        trend_len = length
        trend_mark = 'up'
        pre_value = result.close[i]
        length = 1
...
tc_df = pd.DataFrame(result_trend_counter,columns=['mark','len','rate','date','close'])
...
ax1.plot(down_rate_p.p,color='green')
ax2 = plt.subplot(212)
ax2.plot(up_len_p.p,color='red')
ax2.plot(down_len_p.p,color='green')
